# Generated by Powerlevel10k configuration wizard on 2025-04-01 at 09:21 BST.
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    devshell_lang           # programming language of the current /etc/nixos dev shell
  ...
  ###################[ devshell_lang: programming language of an /etc/nixos dev shell ]###################
  # Renders the language + a per-language Nerd Font glyph when inside one of the
  # /etc/nixos dev shells. These shells export DEVSHELL_LANG (python|go|rust);
  # the segment is hidden whenever that variable is unset. It sits alongside the
  # built-in nix_shell segment, which still shows the NixOS logo and pure/impure.
  function prompt_devshell_lang() {
    local lang=$DEVSHELL_LANG
    [[ -n $lang ]] || return
    local icon label color
    # Icons use $'\uXXXX' escapes (Nerd Font codepoints) so the glyph bytes
    # don't need to live literally in this file.
    case $lang in
      python) icon=$'\ue73c'; label='Python'; color=3 ;;  # yellow
      go)     icon=$'\ue627'; label='Go';     color=6 ;;  # cyan
      rust)   icon=$'\ue7a8'; label='Rust';   color=1 ;;  # red/orange
      *)      icon=$'\uf313'; label=$lang;    color=4 ;;  # fallback: NixOS logo, blue
    esac
    p10k segment -f 0 -b $color -i "$icon" -t "$label"
  }

  # Mirror prompt_devshell_lang for instant prompt so the segment also appears
  # during the brief instant-prompt phase of each new prompt.
  function instant_prompt_devshell_lang() {
    prompt_devshell_lang
  }
